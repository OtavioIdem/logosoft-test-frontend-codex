...
if (existsSync(join(root, 'package.json'))) {
    ...
    const expectedVersion = packageJson.logosoftVersion;
    const scripts = packageJson.scripts ?? {};
    if (!expectedVersion) failures.push('package.json: logosoftVersion é obrigatório');
...
if (existsSync(join(root, 'tests/evidence/integrated-e2e.assisted-evidence.example.json'))) {
    ...
    const expectedVersion = JSON.parse(read('package.json')).logosoftVersion;
    if (evidence.version !== expectedVersion) failures.push(`tests/evidence/integrated-e2e.assisted-evidence.example.json: version deve ser ${expectedVersion}`);
