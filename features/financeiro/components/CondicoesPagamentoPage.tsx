...
export const CondicoesPagamentoPage = () => {
    ...
    return <><PageHeader title="Condições de pagamento" description="Controle parcelamento, intervalo e entrada." actions={<div className="flex flex-column md:flex-row gap-2"><EmpresaFilialFilter empresaId={empresaId} filialId={filialId} onEmpresaChange={(value) => { setEmpresaId(value); setFilialId(null); }} onFilialChange={setFilialId} /><SearchInput ariaLabel="Buscar condições" defaultValue={search} onChange={(term) => { setSearch(term); setFirst(0); }} /><PermissionGuard permission="CONDICOES_PAGAMENTO_GERENCIAR" mode="disable">{({ disabled }) => <Button label="Nova condição" icon="pi pi-plus" disabled={disabled} onClick={() => { setEditing(null); setDialogVisible(true); }} />}</PermissionGuard></div>} />
