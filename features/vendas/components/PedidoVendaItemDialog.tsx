...
import { adicionarItemPedidoVendaSchema, atualizarItemPedidoVendaSchema } from '@/features/vendas/schemas/vendasSchemas';
import { ItemPedidoVendaFormValues, ItemPedidoVendaResponse, PedidoVendaResponse } from '@/features/vendas/types/vendas.types';
import { FieldErrors, fieldErrorMap, formatMoney, localOptions, produtoOptions, textValue } from '@/features/vendas/components/vendasUiUtils';

const buildInitialValues = (item?: ItemPedidoVendaResponse | null): ItemPedidoVendaFormValues =>
    item
        ? { id: item.id, localEstoqueId: item.localEstoqueId ?? null, quantidade: item.quantidade, valorUnitario: item.valorUnitario, valorDesconto: item.valorDesconto, observacao: item.observacao ?? null }
        : { produtoId: '', localEstoqueId: null, quantidade: 1, valorUnitario: 0, valorDesconto: 0, observacao: null };

export const PedidoVendaItemDialog = ({ visible, pedido, item, loading, onHide, onSubmit }: { visible: boolean; pedido: PedidoVendaResponse | null; item?: ItemPedidoVendaResponse | null; loading?: boolean; onHide: () => void; onSubmit: (values: ItemPedidoVendaFormValues) => Promise<void> }) => {
    const [values, setValues] = useState<ItemPedidoVendaFormValues>(() => buildInitialValues(item));
    ...
    const update = (name: keyof ItemPedidoVendaFormValues, value: unknown) => {
        setValues((current) => ({ ...current, [name]: value }));
        setErrors((current) => ({ ...current, [name]: undefined }));
    };

    const submit = async () => {
        const schema = item ? atualizarItemPedidoVendaSchema : adicionarItemPedidoVendaSchema;
        const parsed = schema.safeParse(values);
        if (!parsed.success) {
            setErrors(fieldErrorMap(parsed.error));
            return;
        }
        await onSubmit({ ...values, ...(parsed.data as ItemPedidoVendaFormValues), id: item?.id });
    };
    ...
    return (
        <Dialog header={item ? 'Editar item' : 'Adicionar item'} visible={visible} modal style={{ width: 'min(60rem, 96vw)' }} footer={footer} onHide={onHide}>
            <FormGrid>
